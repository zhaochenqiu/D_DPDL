clear all
close all
clc

addpath('./function/');
addpath('./function/frames_selection/');
addpath('./function/imdb_difference/');
addpath('./function/bayesian/');
addpath('../common/');
addpath('../common_c/');



run('~/libs/matconvnet-1.0-beta25/matlab/vl_setupnn.m')

% 
% im_pa = '~/dataset/dataset2014/dataset/shadow/backdoor/input/';
% im_ft = 'jpg';
% 
% gt_pa = '~/dataset/dataset2014/dataset/shadow/backdoor/groundtruth/';
% gt_ft = 'png';
% 
% fg_pa = '~/projects/matrix/detection/iterative/shadow/backdoor/';
% net_pa = '~/projects/matrix/network/iterative/shadow/backdoor/';
% 
% iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 1);
% 
% 
% 
% 
% clear all
% close all
% clc
% 
% im_pa = '~/dataset/dataset2014/dataset/shadow/bungalows/input/';
% im_ft = 'jpg';
% 
% gt_pa = '~/dataset/dataset2014/dataset/shadow/bungalows/groundtruth/';
% gt_ft = 'png';
% 
% fg_pa = '~/projects/matrix/detection/iterative/shadow/bungalows/';
% net_pa = '~/projects/matrix/network/iterative/shadow/bungalows/';
% 
% iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 1);
% 
% 
% 
% 
% clear all
% close all
% clc
% 
% 
% im_pa = '~/dataset/dataset2014/dataset/shadow/busStation/input/';
% im_ft = 'jpg';
% 
% gt_pa = '~/dataset/dataset2014/dataset/shadow/busStation/groundtruth/';
% gt_ft = 'png';
% 
% fg_pa = '~/projects/matrix/detection/iterative/shadow/busStation/';
% net_pa = '~/projects/matrix/network/iterative/shadow/busStation/';
% 
% iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 1);
% 



clear all
close all
clc


im_pa = '~/dataset/dataset2014/dataset/shadow/copyMachine/input/';
im_ft = 'jpg';

gt_pa = '~/dataset/dataset2014/dataset/shadow/copyMachine/groundtruth/';
gt_ft = 'png';

fg_pa = '~/projects/matrix/detection/iterative/shadow/copyMachine/';
net_pa = '~/projects/matrix/network/iterative/shadow/copyMachine/';

iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 5);


clear all
close all
clc


im_pa = '~/dataset/dataset2014/dataset/shadow/cubicle/input/';
im_ft = 'jpg';

gt_pa = '~/dataset/dataset2014/dataset/shadow/cubicle/groundtruth/';
gt_ft = 'png';

fg_pa = '~/projects/matrix/detection/iterative/shadow/cubicle/';
net_pa = '~/projects/matrix/network/iterative/shadow/cubicle/';

iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 5);


clear all
close all
clc


im_pa = '~/dataset/dataset2014/dataset/shadow/peopleInShade/input/';
im_ft = 'jpg';

gt_pa = '~/dataset/dataset2014/dataset/shadow/peopleInShade/groundtruth/';
gt_ft = 'png';

fg_pa = '~/projects/matrix/detection/iterative/shadow/peopleInShade/';
net_pa = '~/projects/matrix/network/iterative/shadow/peopleInShade/';

iterativelyTrain(im_pa, im_ft, gt_pa, gt_ft, fg_pa, net_pa, 5);



