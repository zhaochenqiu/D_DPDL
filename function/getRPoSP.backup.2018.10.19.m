function re_imdb = getRPoSTP(im_pa, im_ft, gt_pa, gt_ft, curindex, num, mode)
...
% ----------------------------------------------------------------------------------%
                                                                                    %
data = zeros(row_im,column_im,byte_im,num*num,'single');                            %
...
block_im = permute(block_im,[4 3 2 1]);
                                                                                    %
data_tp = reshape(block_im, num, num, byte_im, row_im*column_im);                   %
% ----------------------------------------------------------------------------------%
% ----------------------------------------------------------------------------------%
...
% ----------------------------------------------------------------------------------%
data_RPoSP = zeros(num, num, byte_im*2, row_im*column_im, 'single');                %
...
for i = 1:frames_t
    ...
    for j = 1:byte_im
        im = data_tp(:,:,j,i);
        data_tp(:,:,j,i) = reshape(im(ranidx), row_t, column_t);
    end

    data_RPoSP(:,:,1:3,i) = data_sp(:,:,:,i);
    data_RPoSP(:,:,4:6,i) = data_tp(:,:,:,i);
