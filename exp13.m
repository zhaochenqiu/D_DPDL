...
gt_pa = '~/projects/matrix/result_canoe/';
gt_ft = 'png';


net_path = '../network/net_loop/';
net_pa = [net_path 'net-epoch-460.mat'];
...
imdb = getRPoTP_SG(im_pa, im_ft, gt_pa, gt_ft, 960, 81, 'train', 'set');
imdb = getRPoTP_SG([], [], [], [], [], [], [],'get');

% bgs_train(imdb,'net_test');
bgs_train_recursive(imdb,net_path);
...
imdb = getRPoTP_SG(im_pa, im_ft, gt_pa, gt_ft, 960, 81, 'test', 'set');
...
for i_net = 10:10:400
    ...
    net_pa = [net_path sprintf('net-epoch-%i.mat', i_net)];
    ...
    store_img(:,:, i_net/10) = fgim;
    
    store_all = [store_all store_temp];
