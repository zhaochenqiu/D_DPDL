...
%     gt_ft = 'png';
% 
%     imdb_test = getRPoTP_SG(im_pa, im_ft, gt_pa, gt_ft, 1480, 81, 'test', 'set');
%     imdb_test = getRPoTP_SG([], [], [], [], [], [], [],'get');
% 
% 
%       [fgim gtim] = getFgImg(net, imdb_test);
% 
...
%  input('before clear')
  disp('clear the data for last epoch');
  imdb = [];
  clear imdb;
  params.imdb = [];
  clear params.imdb;
  disp('clear the data for lass epoch completed');
%  input('after clear')

%  pack

%  disp('pack the memery');
%  pack
...
clear imdb.images.data;
imdb.images.data = [];
clear param.imdb.images.data;
param.imdb.images.data = [];
...
%  idx_s = randperm(params.numSubBatches);
%  for cnt_s = 1:params.numSubBatches;
%    s = idx_s(cnt_s);
